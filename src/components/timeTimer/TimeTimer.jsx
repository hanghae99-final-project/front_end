...
const TimeTimer = ({ timeMode, setTimeMode }) => {
  ...
  useEffect(() => {
    remainSec !== 0
      ...
      : setStatus(
          `✏️ 어제 ${changeSecondToTime(
            Math.floor(yesterdayStudyTime / 1000)
          )}공부했어요`
        );
    ...
    <div className={styles.layout}>
      ...
      {!run && !rest ? (
        targetTime === 0 ? (
          ...
          <button
            className={
              second >= targetTime / 1000 ? styles.redBtn : styles.playBtn
            }
            onClick={() => {
              setRefresh(true);
              setRun(true);
          ...
          >
            ...
            <div
              className={
                second >= targetTime / 1000 ? styles.redText : styles.playText
              }
            >
              시작하기
            </div>
